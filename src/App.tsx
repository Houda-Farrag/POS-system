...
export default function App() {
  ...
  const [permissions, setPermissions] = useState<string[]>([]);
  const [loading, setLoading] = useState(true);
  const location = useLocation(); // Add this for debugging
  ...
  return (
    <Layout user={user} arabicMode={arabicMode} setArabicMode={setArabicMode} onLogout={handleLogout} permissions={permissions}>
      <Routes>
        ...
        {/* Phase 2: Purchases Module Routes */}
        {can("purchases:read") && <Route path="/suppliers" element={<SuppliersPage arabicMode={arabicMode} />} />}
        {can("purchases:read") && <Route path="/purchase-orders" element={<PurchaseOrdersPage arabicMode={arabicMode} />} />}
        {can("purchases:read") && <Route path="/goods-receiving" element={<GoodsReceivingPage arabicMode={arabicMode} />} />}
        {can("purchases:read") && <Route path="/supplier-payments" element={<SupplierPaymentsPage arabicMode={arabicMode} />} />}
